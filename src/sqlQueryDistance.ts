...
export async function calculateDistance(
    ...
    let queue: Map<number, Node[]> = new Map();
    ...
    partsLoop: for(let currentPart = 0; currentPart < num_parts; currentPart++){
        ...
        queueLoop: while(!canceled) {
            ...
            let factor = edit.perform(query, schema, metaInfo, neighbors) as unknown as number | undefined;
            
            factor  = factor == undefined? 0: factor;
            let variableCost = 0;
            if (factor > 0) variableCost = (factor - 1) * edit.cost;
            

            /* if(edit.name.startsWith('addSelect') && neighbors.length > 1 ){
                console.log('-------------')
                console.log('variableCost', variableCost);
                console.log(edit.name)
                console.log(stringifyQuery(query));
                console.log(neighbors.length, 'new neighbours')
            ...
            neighborLoop: for(let n=0, nl=neighbors.length; n<nl; ++n) {
                ...
                if(destination.equals(neighbor)) {
                    ...
                    destinationNode = new Node(neighbor, currentDistance + variableCost, node, edit);
                    break partsLoop;
                }
                //mark this neighbor as visited
                ++queryCount;
                const neighborNode = new Node(neighbor, currentDistance + variableCost, node, edit);
